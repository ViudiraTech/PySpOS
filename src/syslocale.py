...
CATALOGS = {
    "zh_CN": {
        ...
        "oobe.bl_b": "Bootloader 当前：{state}。\n上锁时提权仅临时有效（重启失效）；解锁需通过 Token 验证（open unlock）。",
        "oobe.ota_t": "OTA 更新通道",
        "oobe.ota_b": "选择更新通道（云端恢复前仅做记录，恢复后生效）。",
        "oobe.ota_stable": "stable（仅正式版）",
        "oobe.ota_beta": "beta（抢先体验）",
        "oobe.token_t": "获取解锁 Token",
        "oobe.token_b": "进入系统后执行 open gettoken，通过答题获取 Bootloader 解锁 Token。\nToken 请勿泄露，ROOT 相关操作会被记入审计日志。",
        "oobe.summary_t": "确认配置",
    ...
    "en": {
        ...
        "oobe.bl_b": "Bootloader is {state}.\nWhile locked, privilege escalation is temporary (lost on reboot); unlocking needs token verification (open unlock).",
        ...
        "oobe.token_t": "Unlock token",
        "oobe.token_b": "After entering the system, run open gettoken and pass the quiz to get the bootloader unlock token.\nKeep it secret; ROOT actions are audit-logged.",
