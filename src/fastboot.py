...
def perform_unlock() -> tuple:
    ...
    try:
        ...
        trusted = _device_trusted_keys()
        secure_boot.write_policy(root, False, 0, device_key_path(), trusted)
        # The policy is signed by the device key, so the runtime trust store
        # has to learn that key again or the next read would fail closed and
        # report the device as locked again.
        secure_boot.configure_runtime_keys(root, False)
    except Exception as exc:
        return False, f"签发解锁策略失败: {exc}"
...
def perform_lock() -> tuple:
    ...
    try:
        secure_boot.ensure_developer_key(root, locked=False)
        trusted = _device_trusted_keys()
        secure_boot.write_policy(root, True, rollback_index(),
                                 device_key_path(), trusted)
        secure_boot.configure_runtime_keys(root, True)
    except Exception as exc:
        return False, f"签发锁定策略失败: {exc}"
...
# Stage the staged image as the next boot target, mirroring "boot".
def perform_boot() -> tuple:
    if not staged_bytes():
        return False, "没有已下载的镜像"
    set_pending_boot("staged")
    return True, "已安排启动已下载的镜像"
